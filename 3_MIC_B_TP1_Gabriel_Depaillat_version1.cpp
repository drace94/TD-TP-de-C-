...
class File{
	...
	public :
		...
		Figure * premier_element() {
			if (file_vide()) {throw Vide("File vide");}
		...
		class Vide : public std::runtime_error {
			public:
				Vide(const char * msg) : std::runtime_error(msg) {}
...
int main(void) {
	...
	*/
    	...
    	File file;
    	file.insertion(new Rectangle(4.,3.));
    	file.insertion(new Carre(2.));
    	file.insertion(new TriangleEquilateral(3.));
    	file.insertion(new Cercle(2.,"red"));
    	file.insertion(new Cercle(3.,"blue"));
	...
	file.suppression(); //ca leve l'exception donc ok
	return(0);
}
